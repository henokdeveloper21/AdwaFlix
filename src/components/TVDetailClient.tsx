...
// ─── Helper to group sources by provider ──────────────────
function groupSourcesByProvider(sources: any[]) {
  const grouped: Record<string, any[]> = {};
  sources.forEach((s) => {
    const key = typeof s.provider === 'string' ? s.provider : (s.provider?.name || s.provider?.id || 'Unknown');
...
export function TVDetailClient({ showData, showId }: TVDetailClientProps) {
    ...
    const fetchExtra = async () => {
      try {
        const [videos, similar] = await Promise.all([
          tmdbService.getVideos("tv", showId),
          tmdbService.getSimilar("tv", showId),
        ]);
        ...
        setSimilarShows(similar.results.slice(0, 8));

        setInFavorites(isInFavorites(showId));
        setInWatchlist(isInWatchlist(showId));
      } catch {}
    };
    fetchExtra();
  }, [showId, isInFavorites, isInWatchlist]);
  ...
  const fetchProviderSources = useCallback(async () => {
    ...
    try {
      ...
      const year = getYearFromDate(showData.first_air_date);
      ...
      // Normalize provider field to always be a string
      const normalizedSources = (data.sources || []).map((s: any) => ({
        ...s,
        provider: typeof s.provider === 'string' ? s.provider : s.provider?.name || s.provider?.id || 'Unknown',
      }));

      // Validate sources – only keep playable ones
  ...
  }, [showData, showId, selectedSeason, selectedEpisode]);
  ...
  const playSource = (source: any) => {
    const params = new URLSearchParams({
      url: source.url,
      title: `${showData.name} S${selectedSeason} E${selectedEpisode}`,
      type: 'tv',
      id: showId.toString(),
      season: selectedSeason.toString(),
      episode: selectedEpisode.toString(),
  ...
  const seasonOptions = Array.from(
    { length: showData?.number_of_seasons || 0 },
    (_, i) => i + 1
  );
  ...
  // Group sources by provider for the sheet
  const groupedProviders = groupSourcesByProvider(streamSources);

  const backdropUrl = showData
    ? tmdbService.getBackdropUrl(showData.backdrop_path, "w1280")
    : "";
  ...
  const jsonLd = showData ? {
    ...
    startDate: showData.first_air_date,
    aggregateRating: showData.vote_average ? {
      '@type': 'AggregateRating',
      ratingValue: showData.vote_average,
      bestRating: 10,
      ratingCount: showData.vote_count,
    } : undefined,
      ...
      <div className="container-premium -mt-64 relative z-20">
        ...
        >
              ...
              {seasonOptions.length > 0 && (
                <div className="grid grid-cols-2 gap-2">
                  <div className="relative">
                    <label className="text-xs text-zinc-400 block mb-1">Season</label>
                    <select
                      value={selectedSeason}
                      onChange={(e) => setSelectedSeason(Number(e.target.value))}
                      className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-white text-sm outline-none focus:border-amber-500/50 appearance-none cursor-pointer"
                    >
                      {seasonOptions.map((s) => (
                        <option key={s} value={s} className="bg-black text-white">Season {s}</option>
                      ))}
                    </select>
                    <ChevronDown className="absolute right-2 bottom-2 text-zinc-400 pointer-events-none" size={14} />
                  </div>
                  <div className="relative">
                    <label className="text-xs text-zinc-400 block mb-1">Episode</label>
                    <select
                    ...
                    >
                      {episodeOptions.map((e) => (
                        <option key={e} value={e} className="bg-black text-white">Episode {e}</option>
                      ))}
                    </select>
                    <ChevronDown className="absolute right-2 bottom-2 text-zinc-400 pointer-events-none" size={14} />
                  </div>
                  ...
                  <div className="p-6 space-y-6">
                    ...
                    {providersLoading && !validating && (
                      <div className="flex items-center justify-center py-8">
                        ...
                        <span className="ml-3 text-zinc-400">Fetching streams...</span>
                      </div>
                    )}
                    ...
                    {providersError && (
                      <div className="text-center py-4">
                        ...
                        <Button variant="glass" size="sm" onClick={fetchProviderSources}>
                          Retry
                        </Button>
                      </div>
                    )}

                    {!providersLoading && !validating && !providersError && streamSources.length === 0 && (
                      <div className="text-center py-4">
                        <Wifi className="mx-auto text-zinc-500 mb-2" size={32} />
                        <p className="text-zinc-400">No playable sources found for this episode.</p>
                        <p className="text-zinc-500 text-xs mt-1">All available streams were unreachable or blocked.</p>
                      </div>
                    )}
                    ...
                    {streamSources.length > 0 && (
                      <div className="space-y-3">
                        {Object.entries(groupedProviders).map(([providerName, sources]) => (
                          <div key={providerName} className="rounded-xl bg-white/5 border border-white/10 overflow-hidden hover:border-amber-500/20 transition-all">
                            <button
                            ...
                            >
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  ...
                                  <p className="text-xs text-zinc-400">{sources.length} healthy stream{sources.length > 1 ? 's' : ''}</p>
                                </div>
                              </div>
                              <ChevronRight
                                size={18}
                                className={`text-zinc-400 transition-transform duration-200 ${
                                  expandedProvider === providerName ? 'rotate-90' : ''
                                }`}
                              />
                            </button>
                            <AnimatePresence>
          ...
          <div className="md:col-span-3 space-y-8">
              ...
              <div className="flex flex-wrap items-center gap-4 text-sm">
                <div className="flex items-center gap-2 bg-amber-500 rounded-full px-4 py-2">
                  ...
                  <span className="font-bold text-black">
                    {formatRating(showData.vote_average)}
                  </span>
                </div>
                <span className="text-zinc-400">
                  {getYearFromDate(showData.first_air_date)}
                </span>
                <span className="text-zinc-400">
                  {showData.number_of_seasons} season
                  {showData.number_of_seasons !== 1 ? "s" : ""}
                </span>
                <span className="text-zinc-400">
            ...
            {showData.credits?.cast?.length > 0 && (
              <div>
                <h3 className="text-xl font-bold text-white mb-4">Cast</h3>
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                  {showData.credits.cast.slice(0, 8).map((actor: any) => (
            ...
            >
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                {similarShows.map((show, i) => (
                  ...
                  >
                    <MovieCard media={show} isTV index={i} />
                  </motion.div>
                ))}
              </div>
            </motion.div>
